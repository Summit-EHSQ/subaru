...
Suppliers currently interact with numerous SIA systems that use different navigation patterns and interfaces. The existing supplier portal acts mainly as a redirection point and does not provide a complete, consolidated entry experience. Intelex will not replace every external supplier-facing system, but the program requires a recognizable home from which suppliers can find Intelex work, training, documents, and links to systems that remain outside Intelex.
...
### Use each application as a separate supplier entry point

Preserves existing boundaries but continues fragmented navigation and training.
...
### Use the Intelex Supplier Portal as the official landing page and link to external systems

Creates a unified experience while respecting application boundaries.

## Decision Outcome

Use the Intelex Supplier Portal, anchored by Supplier Relationship Management, as the official supplier landing page. Supplier-facing processes delivered in Intelex will be available directly from that portal. Systems that remain outside Intelex will be represented through clearly organized links, supporting information, and training content.
